...
public partial class Editor : CanvasLayer
{
    ...
    public ISpatialReference SelectedReference
    {
    ...
    private void ApplyLevelData(LevelData data)
    {
        ...
        _timeline.Load(data.Projectiles, data.Patterns, levelData.Duration);
        _preview.Load(data);
    ...
    public void AddReference(ISpatialReference reference)
    {
        ...
        if (reference is ProjectileReference pr)
        {
            levelData.Projectiles.Add(pr);
            _preview.AddInstance(pr);
        }
        else if (reference is PatternReference ptr)
        {
            levelData.Patterns.Add(ptr);
            _preview.AddInstance(ptr);
        }
    }
    public void DeleteReference(ISpatialReference reference)
    {
        _timeline.RemoveMarker(reference);
        if (reference is ProjectileReference pr)
        {
            levelData.Projectiles.Remove(pr);
            _preview.RemoveInstance(pr);
        }
        else if (reference is PatternReference ptr)
        {
